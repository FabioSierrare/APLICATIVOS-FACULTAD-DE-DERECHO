...
export default function DatePickerWithBlocks({
  ...
  const selectedValue = typeof selected === "undefined" ? internalSelected : selected;
  ...
  const [mesVisible, setMesVisible] = useState(new Date()); // empieza en el mes actual
  ...
  const cargarFestivos = async () => {
    ...
    console.log(festivo); // Aquí sí tendrás el Array(23)
  };
...
const handleMonthChange = (month) => {
  ...
  const isBlocked = (date) => {
    ...
    if(day.getDay() === festivo.fecha)
    //bloquea el domingo
    if(day.getDay() === 0) return true
    //bloquea el sabado
    if(day.getDay() === 6) return true
    // 1) Si es martes -> bloquear (2 = martes)
    if (day.getDay() === diasSemana.indexOf(calendario.diaConciliacion) + 1) return true;
  ...
  // Para debug: imprime las fechas normalizadas en consola
  // (quita en producción)
  console.debug("blockedNormalized:", blockedNormalized);
  ...
  return (
      ...
      <div className={`xs:p-4 p-0 rounded-2xl shadow-lg border-white border-2 bg-primary flex flex-col xs:max-w-85 max-w-full ${className} uppercase`}>
          <DayPicker
          locale={es}
          mode={mode}
