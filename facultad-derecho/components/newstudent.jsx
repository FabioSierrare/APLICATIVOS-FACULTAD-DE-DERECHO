export default function SelectedFileExcel({onFileSelect}){
    function handleFileChange(e) {
    const file = e.target.files[0];
    if (!file) return;

    // Verifica que sea Excel
    if (!file.name.endsWith(".xlsx") && !file.name.endsWith(".xls")) {
      alert("El archivo debe ser Excel (.xlsx o .xls)");
      return;
    }

    // Enviar el archivo al componente padre
    onFileSelect(file);
  }

    return <div className="flex flex-col text-white mb-5">
            <label className="mb-3 flex gap-2" htmlFor="Archivo excel">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 640 640"
                className="w-6 text-white "
                fill="currentColor"
              >
                <path d="M192 112L304 112L304 200C304 239.8 336.2 272 376 272L464 272L464 512C464 520.8 456.8 528 448 528L192 528C183.2 528 176 520.8 176 512L176 128C176 119.2 183.2 112 192 112zM352 131.9L444.1 224L376 224C362.7 224 352 213.3 352 200L352 131.9zM192 64C156.7 64 128 92.7 128 128L128 512C128 547.3 156.7 576 192 576L448 576C483.3 576 512 547.3 512 512L512 250.5C512 233.5 505.3 217.2 493.3 205.2L370.7 82.7C358.7 70.7 342.5 64 325.5 64L192 64zM291.2 329.6C283.2 319 268.2 316.8 257.6 324.8C247 332.8 244.8 347.8 252.8 358.4L290 408L252.8 457.6C244.8 468.2 247 483.2 257.6 491.2C268.2 499.2 283.2 497 291.2 486.4L320 448L348.8 486.4C356.8 497 371.8 499.2 382.4 491.2C393 483.2 395.2 468.2 387.2 457.6L350 408L387.2 358.4C395.2 347.8 393 332.8 382.4 324.8C371.8 316.8 356.8 319 348.8 329.6L320 368L291.2 329.6z" />
              </svg>
              Agregar listados de estudiantes
            </label>
            <input
              type="file"
              id="archivoExcel"
              accept=".xlsx, .xls"
              onChange={handleFileChange}
              className="bg-white text-black/75 p-1 rounded-xl"
            />
          </div>
}